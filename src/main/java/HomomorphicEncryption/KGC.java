...
public class KGC {
    ...
    public  int pkSetSize = 20; //감마 + 람다 (but 너무 커서 일단 감마^3+람다로)
    ...
    private  BigInteger alpha = new BigInteger("174682bb762fb605edd9dea02a610fd6cc6ad6ea2b",16); //system alpha
    ...
    /*
         ...
         */
        pkSet.clear();
        if(HEDataBase.hasSeverPublicKey()){
            for(int i = 0; i<pkSetSize; i++){
                if(i == 0){
                    pkSet.add(HEDataBase.getServerPublicKeyX0());
                }
                else{
                    pkSet.add(HEDataBase.getServerPublicKey(i));
                }
            }
            return;
        }
        ...
        System.out.println("a(16) = " + alpha.toString(16));

        //pkSet 뽑기 (kgc의 공개키)
        BigInteger qMax = new BigInteger("2").pow(gamma.intValue()).divide(p);
        for(int i = 0; i<pkSetSize; i++){
            BigInteger q = new BigInteger(qMax.bitLength()-1,random);
            BigInteger r = new BigInteger(lamda.intValue()-1,random).multiply(new BigInteger("-1").pow(new BigInteger(1,random).intValue()));
            BigInteger pk = p.multiply(q).add(r);
            pkSet.add(new AGCDPublicKey(pk,q,r));
        }

        Collections.sort(pkSet); //X0 is the largest element

        //조건에 맞는 xo만들기
        pkSet.set(0,checkX0Condition(pkSet.get(0), alpha));
        ...
        HEDataBase.uploadServerPublickey(pkSet);
    }

    //au 값 설정 후 return
    public BigInteger shareAlpha(){
        BigInteger au = BigInteger.ONE;
        for(int i=0;i<lamda.intValue();i++){
            au = au.multiply(BigInteger.TWO);
        }
        au = au.add(new BigInteger(lamda.intValue()-1,random)).nextProbablePrime();//최대 2^19
        return au;
    }
    public AGCDPublicKey checkX0Condition(AGCDPublicKey x0, BigInteger alpha){
        System.out.println("checkX0Condition 함수 진입");
        if (!x0.pk.mod(p).equals(alpha)){ //X0 mod p = a 조건 체크
            ...
            if(rest.compareTo(p)<0){
                x0.setR(rest);
            }
            else{
                x0.setQ(rest.divide(p));
                x0.setR(rest.mod(p));
            }
            x0.setPk(p);
    ...
    public BigInteger getSystemAlpha() {
        return alpha;
    }
}
