...
//        senderCipherText.setVisible(false);
    ...
    private void recipientDecryptButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_recipientDecryptButtonActionPerformed
        try {
//            byte[] cipherText = Base64.decode(recipientCipherTextArea.getText());
...
//            recipientDecryptionResultTextArea.setText(new String(plainText));
            byte[] plainText = recipientDecrypt(recipientCipherTextArea.getText());
            recipientDecryptionResultTextArea.setText(new String(plainText));
    ...
    private void senderEncryptButtonActionPerformed(java.awt.event.ActionEvent evt) {
        //GEN-FIRST:event_senderEncryptButtonActionPerformed
//        byte[] recipientPublicKey = Hex.decode(senderRecipientPublicKeyEditText.getText());
//        byte[] recipientPublicKey = Base64.decode(senderRecipientPublicKeyEditText.getText());
//
...
//        System.arraycopy(chiperText, 0, result, R.length, chiperText.length);
        byte[] result = senderEncrypt(senderRecipientPublicKeyEditText.getText(),senderPlainTextArea.getText());
        senderCipherTextArea.setText(Base64.toBase64String(result));
    ...
    //암호문 decrypt
    private byte[] recipientDecrypt(String cipher) throws Exception {
        byte[] cipherText = Base64.decode(cipher);
        byte[] receiverR = new byte[ecies.getKeySize()];
        System.arraycopy(cipherText, 0, receiverR, 0, ecies.getKeySize());
    ...
    private byte[] senderEncrypt(String publicKey, String plainText) {
    ...
    //암호문을 데베에 저장하는 함수 추가하기 ( 계약 중의 상황)
    //pem파일로 공개키 개인키 저장하는 함수도 추가하기
}
